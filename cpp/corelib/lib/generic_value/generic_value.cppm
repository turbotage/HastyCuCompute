...
public:
	...
	template<is_device D, is_tensor_type T, size_t R>
	explicit generic_value(tensor<D,T,R> ten)
		: m_type(value_type::TENSOR), m_data(ten.get_tensor()) {}
	...
	static generic_value from_tensor_container(T&& container) {
		if constexpr (hasty::is_tensor<std::remove_cvref_t<T>>) {
			return generic_value(std::forward<T>(container));
		}
	...
	template<is_tensor_container T>
	T to_static() const {
		if constexpr (hasty::is_tensor<T>) {
		...
		else if constexpr (is_tensor_vector<T>) {
		...
		else if constexpr (is_tensor_dict<T>) {
		...
		else if constexpr (is_tensor_tuple<T>) {
...
public:
	...
	/**
	 * @brief Serialize the entire value to a binary stream
	 * @param os Output stream to write to
	 */
	void serialize(std::ostream& os) const {
		// Write type tag
		u8 type_tag = static_cast<u8>(m_type);
		os.write(reinterpret_cast<const char*>(&type_tag), sizeof(type_tag));
		...
		switch (m_type) {
			...
			case value_type::TENSOR:
				serialize_tensor(os, std::get<hat::Tensor>(m_data));
				break;
	...
	/**
	 * @brief Deserialize a value from a binary stream
	 * @param is Input stream to read from
	 * @return Deserialized generic_value
	 */
	static generic_value deserialize(std::istream& is) {
		// Read type tag
		u8 type_tag;
		is.read(reinterpret_cast<char*>(&type_tag), sizeof(type_tag));
		auto type = static_cast<value_type>(type_tag);
		...
		switch (type) {
			...
			case value_type::TENSOR:
				result.m_data = deserialize_tensor(is);
				break;
...
private:
	// Helper: Serialize tensor to stream
	static void serialize_tensor(std::ostream& os, const hat::Tensor& tensor) {
		...
		for (int i = 0; i < tensor.dim(); ++i) {
			i64 size = tensor.size(i);
			os.write(reinterpret_cast<const char*>(&size), sizeof(size));
		}
		
		// Write data (ensure it's contiguous and on CPU)
		hat::Tensor cpu_tensor = tensor.to(hat::kCPU).contiguous();
		size_t data_size = cpu_tensor.numel() * cpu_tensor.element_size();
		os.write(reinterpret_cast<const char*>(cpu_tensor.data_ptr()), data_size);
	}
	
	// Helper: Deserialize tensor from stream
	static hat::Tensor deserialize_tensor(std::istream& is) {
		serialized_tensor_header header;
		is.read(reinterpret_cast<char*>(&header), sizeof(header));
		
		// Read shape
		std::vector<i64> shape(header.ndim);
		for (int i = 0; i < header.ndim; ++i) {
			is.read(reinterpret_cast<char*>(&shape[i]), sizeof(i64));
		}
		...
		is.read(reinterpret_cast<char*>(tensor.data_ptr()), data_size);
...
export htorch::jit::IValue generic_to_ivalue(generic_value&& gvalue) {
	if (gvalue.is_none()) {
		...
		ivalue_map.reserve(dict.size());
		for (auto& [key, value] : dict) {
			ivalue_map.emplace(key, generic_to_ivalue(std::move(value)));
		}
		
		// Get the value type from the first element
		auto value_typeptr = ivalue_map.begin()->second.type();
		hc10::impl::GenericDict ivalue_dict(hc10::StringType::get(), value_typeptr);
		ivalue_dict.reserve(ivalue_map.size());
		for (auto& [key, ivalue_elem] : ivalue_map) {
			if (ivalue_elem.type() != value_typeptr) {
				throw std::runtime_error("Inconsistent types in generic_to_ivalue dict");
			}
			ivalue_dict.insert(key, std::move(ivalue_elem));
		}
		return htorch::jit::IValue(std::move(ivalue_dict));
	} else if (gvalue.is_tuple()) {
		const auto& vec = gvalue.to_tuple();  // Tuple uses vector storage (const ref)
		std::vector<htorch::jit::IValue> ivalue_elements;
		ivalue_elements.reserve(vec.size());
		// Need to copy each elem since vec is const&
		for (size_t i = 0; i < vec.size(); ++i) {
			ivalue_elements.push_back(generic_to_ivalue(std::move(const_cast<generic_value&>(vec[i]))));
		}
