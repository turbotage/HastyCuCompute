...
namespace hasty {
	...
	namespace fft {
		...
		/*'
		Toeplitz multiplication kernel, this is the toeplitz multiply when using the straight kernel the toeplitz_kernel builder
		above, if the multiplication methods in toeplitz.cppm are to be used, the kernel first needs to be transformed to the right 
		format using the toeplitz_kernel_transform function in toeplitz.cppm
		*/
		export template<is_fp_complex_tensor_type TT, size_t DIM>
		requires is_dim3<DIM>
