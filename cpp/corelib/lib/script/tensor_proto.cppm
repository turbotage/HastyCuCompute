...
namespace hasty {
    namespace script {
        ...
        // Specialization for std::map with string or int keys
        template<typename K, typename V, int Depth>
        requires (Depth < 10) && (std::same_as<K, std::string> || std::same_as<K, std::size_t>)
        struct is_tensor_prototype_container_impl<std::map<K, V>, Depth> {
        ...
        template<typename T>
        concept is_tensor_prototype_container = is_tensor_prototype_container_impl<T>::value;
        ...
        // ====================
        template<typename T, int Depth>
        requires (Depth < 10) && (is_tensor_prototype_container_impl<T, Depth + 1>::value || 
                                is_tensor_container_impl<T, Depth + 1>::value)
        ...
        // ====================
        // std::map - automatically detects direction
        // ====================
        template<typename K, typename V, int Depth>
        requires (Depth < 10) && (std::same_as<K, std::string> || std::same_as<K, std::size_t>) &&
                (is_tensor_prototype_container_impl<V, Depth + 1>::value || 
                is_tensor_container_impl<V, Depth + 1>::value)
        struct tensor_prototype_container_conversion<std::map<K, V>, Depth> {
            using inner_type = typename tensor_prototype_container_conversion<V, Depth + 1>::type;
            using type = std::map<K, inner_type>;
            
            type operator()(const std::map<K, V>& map) const {
                type result;
        ...
        // ====================
        template<typename... Ts, int Depth>
        requires (Depth < 10) && ((is_tensor_prototype_container_impl<Ts, Depth + 1>::value || 
                                is_tensor_container_impl<Ts, Depth + 1>::value) && ...)
